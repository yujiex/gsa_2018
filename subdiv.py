...
for csv in filelist[303:304]:
    ...
    df['Usage_daily'] = df['Usage/Quantity']/df['days']
    df['Cost_daily'] = df['Cost ($)']/df['days']
    df = df.set_index(pd.DatetimeIndex(df['Start Date']))
    df.drop(['Usage/Quantity', 'End Date', 'days', 'Cost ($)'],
            axis=1, inplace=True)
    grouped = df.groupby('Meter Type')
    #print grouped.size()

    # upsample data
    outdir = os.getcwd() + '/csv/test_sub/'
    def get_subdir(name):
    ...
    for name, group in grouped:
        ...
        group_up = group.resample('M', how = 'sum', fill_method = 'pad')
        ...
        group_up.to_csv(outdir + get_subdir(name) + outfilename,
                        index=True, index_label='Start Date')
