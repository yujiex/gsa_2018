    ...
    if kind != 'all':
        ...
        if ld == 'line':
            ...
            for name, group in gr_base:
                ...
                tempdf_gas['base_month'] = tempdf_gas['month'].map(lambda x: True if (6 <= x and x <= 8) else False)
                tempdf_gas = tempdf_gas[tempdf_gas['base_month'] == True]
                print tempdf_gas
                base_gas_dict[name] = tempdf_gas['eui_gas'].mean()

                tempdf_elec['base_month'] = \
                        tempdf_elec['month'].map(lambda x: True if x == 12 or x < 3 else False)
            ...
            g = sns.lmplot(x='dd', y='eui_plot', data=df_all, col='Fiscal Year', hue = 'kind', fit_reg=True, truncate=True, order=2)
            #plt.xlabel(xlabel_dict[kind], fontsize=12)
