...
querystr = paste0("SELECT * FROM EUAS_monthly WHERE [Building_Number] = 'CA0198ZZ'")
dbGetQuery(con,  querystr) %>%
  as_data_frame() %>%
  dplyr::select(`year`, `month`, `Electricity_(KWH)`, `Gross_Sq.Ft`) %>%
  dplyr::mutate(`Date`=zoo::as.Date(zoo::as.yearmon(paste(`year`, `month`), "%Y %m"))) %>%
  ggplot() +
  geom_line(aes(y=`Electricity_(KWH)`, x=`Date`)) +
  geom_line(aes(y=`Gross_Sq.Ft`, x=`Date`)) +
  ggtitle("Building CA0198ZZ Electricity KWH") +
  theme()
...
querystr = paste0("SELECT DISTINCT State, [Region_No.] FROM EUAS_monthly WHERE [Region_No.] = '11'")
dbGetQuery(con,  querystr) %>%
  as_data_frame() %>%
  head()

setwd("~/Downloads/")
readr::read_csv("cmu_data.csv")
