...
getWeightedHourly <- function(b, start_str, end_str, v, path=NULL, radius=100) {
  ...
  isd_data_var = isd_data_full
  if (!is.null(path)) {
    dir.create(sprintf("%s/building_%s", path, v))
  } else {
    dir.create(sprintf("building_%s", v))
  }
  print("path in getWeightedHourly")
  print(path)
  download_rounds = length(start_times)
  ## first run to download data
  get.noaa.weather::run_download_isd(b, download_rounds, start_times, end_times,
                                     v, isd_data_var, radius=radius, path=path)
