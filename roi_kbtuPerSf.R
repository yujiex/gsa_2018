## hard coded removing 0 energy consumptions, might look into removing trailing
## 0's
...
compute_roi <- function(building, projectDate) {
  ...
  dfPost = dfElec %>%
    ...
    head(n=36) %>%
    {.}
  elecPost = sum(dfPost$Electric) / nrow(dfPost) * 12
  ...
  dfPost %>%
    ...
    readr::write_csv(paste0("csv_FY/tempResultsInRoi/", building, "_elec.csv"))
  ...
  temp %>%
    ...
    theme()
  ggsave(file=paste0("images/", building, "_trend_elec_.png"), width=4, height=4, units="in")
  ...
  dplyr::bind_rows(dfPre, dfPost) %>%
    ...
    theme()
  ggsave(file=paste0("images/", building, "_reg_elec_.png"), width=4, height=4, units="in")
  ...
  dfPost %>%
    ...
    readr::write_csv(paste0("csv_FY/tempResultsInRoi/", building, "_gas.csv"))
  ...
  ggsave(file=paste0("images/", building, "_trend_gas_.png"), width=4, height=4, units="in")
  ...
  dplyr::bind_rows(dfPre, dfPost) %>%
    ...
    theme()
  ggsave(file=paste0("images/", building, "_reg_gas_.png"), width=4, height=4, units="in")
  return(list("elec"=elecSaving, "gas"=gasSaving, "elecPercent"=(100) * elecSaving / elecPost, "elecTotal"=elecPost,
              "gasPercent"=(100) * gasSaving / gasPost, "gasTotal"=gasPost, "area"=area))
}
...
## for (i in c(6, 8, 10)) {
  ...
  print(sprintf("%s,%s,%.0f,%.2f,%.2f%%,%.2f,%.2f,%.2f%%,%.2f", building, projectDate, result$area, result$elec, result$elecPercent, result$elecTotal, result$gas, result$gasPercent, result$gasTotal, sep=","))
}
